...
| `<think>...</think>` lẫn vào `generated_answer`, và các judge (Correctness/ISSUP/ISUSE/ISREL/Retrieve-decision) parse JSON sai âm thầm — rơi về nhãn mặc định | Đã gặp thật, phát hiện qua thống kê 100% một nhãn trên 213 câu (xem `RESULTS_ANALYSIS.md` §2). Nguyên nhân: `qwen/qwen3.6-27b` là model "thinking", luôn bọc suy luận trước output kể cả khi yêu cầu chỉ trả JSON. Đã thêm `strip_think()` (cắt trước khi parse JSON và trước khi lưu answer) ở cả 3 notebook 02/03/04, và hạ `qwen` xuống cuối `CANDIDATE_MODELS`. Nếu suy luận bị cắt cụt do hết token trước khi đóng `</think>`, không cứu được câu trả lời đó — chấp nhận như một giới hạn còn lại |
...
- [x] `04_evaluation_report.ipynb` — sinh Hệ No-RAG, chấm Correctness/Support/Usefulness thống nhất cho cả 3 hệ trên phần giao nhau đã chạy xong, xuất `final_comparison_table.csv`. **Đã vá lỗi `<think>` (xem `RESULTS_ANALYSIS.md` §2) — cần xoá `evaluation_details.jsonl`/`final_comparison_table.csv` cũ trên Drive và chạy lại notebook này trước khi dùng số liệu Correctness/Support/Usefulness cho báo cáo.**
