...
| `404 model not found` cho toàn bộ `CANDIDATE_MODELS` khi vừa chuyển sang Gemini | Đã gặp thật ngay lần đầu (2026-08-26) — `gemini-2.0-flash`/`gemini-2.0-flash-lite` đã bị Google khai tử, xác nhận qua https://ai.google.dev/gemini-api/docs/models. Sửa: cập nhật `CANDIDATE_MODELS` sang dòng model hiện hành + thêm cơ chế tự động chọn model từ `client.models.list()` thật của tài khoản nếu cả danh sách ưu tiên đều không dùng được (không phải chỉ đổi tên 1 lần, để tránh lặp lại sự cố này lần thứ 3) |
