...
| **Correctness ≈ 0.5 điểm cho cả 3 hệ, nhưng KHÔNG phải do `<think>`** (đã gặp thật lần 2, 07/09/2026, sau khi lỗi `<think>` ở trên đã giảm hẳn) | Nguyên nhân khác: Notebook 4 tốn 6 lệnh gọi LLM/câu (nhiều nhất trong cả pipeline) — nếu chạy sau khi Notebook 2/3 đã dùng phần lớn quota Groq trong cùng ngày, quota cạn hẳn chỉ sau 1-2 câu đầu, khiến gần như toàn bộ các câu còn lại bị `judge_*()` âm thầm rơi về nhãn mặc định (chứng minh được: chỉ 2/202 dòng đầu trong `evaluation_details.jsonl` có nhãn thật, 200 dòng còn lại default 100%). Đã sửa notebook 04 §8: tự dừng sớm khi hết quota (không ghi rác), lưu `_reason` từng judge để dễ chẩn đoán, tự in cảnh báo `CANH BAO` nếu phát hiện câu bị mặc định toàn bộ. **Bài học vận hành**: chạy Notebook 4 vào phiên/ngày riêng, lúc quota Groq còn nguyên — không chạy ngay sau Notebook 2/3 cùng ngày. Chi tiết đầy đủ ở `RESULTS_ANALYSIS.md` |
...
- [x] `04_evaluation_report.ipynb` — sinh Hệ No-RAG, chấm Correctness/Support/Usefulness thống nhất cho cả 3 hệ trên phần giao nhau đã chạy xong, xuất `final_comparison_table.csv`. **Đã vá 2 lỗi riêng biệt (xem `RESULTS_ANALYSIS.md`): lỗi `<think>` (26/08/2026, đã giảm hẳn) và lỗi quota Groq cạn giữa chừng làm hỏng gần hết judge (07/09/2026, đã thêm dừng sớm + log `_reason` + cảnh báo tự động) — cần xoá `evaluation_details.jsonl`/`final_comparison_table.csv` cũ trên Drive, và chạy lại notebook này **vào lúc quota còn nguyên, tách riêng khỏi lần chạy Notebook 2/3** trước khi dùng số liệu Correctness/Support/Usefulness cho báo cáo.**
