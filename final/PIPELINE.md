...
Paper gốc train một critic model + fine-tune generator (7B+) để tự sinh reflection token. Trên Colab free (T4 15GB, session giới hạn giờ, không đảm bảo persistent runtime), việc này không khả thi. Pipeline dùng **Gemini API (free tier qua Google AI Studio, không yêu cầu billing nếu lấy key đúng luồng)** qua package `google-genai`. `GENERATOR_MODEL` không hardcode — notebook thử `client.models.list()` để xác nhận model thật sự khả dụng, ưu tiên **alias `-latest`** do Google tự quản lý (`gemini-flash-lite-latest` → `gemini-flash-latest` → `gemini-pro-latest`), dự phòng bằng tên cụ thể (`gemini-2.5-flash` → `gemini-2.5-flash-lite` → `gemini-2.5-pro`); nếu **không model nào** trong danh sách này còn khả dụng, tự động chọn model text-generation khác từ danh sách thật của tài khoản thay vì dừng lại. Đã cần đến cơ chế này 2 lần liên tiếp trong cùng 1 ngày khi mới chuyển sang Gemini (2026-08-26, xem §6.4): lần 1 `gemini-2.0-flash`/`gemini-2.0-flash-lite` bị Google khai tử (404); lần 2 `gemini-2.5-flash`/`gemini-3.5-flash` bị chặn ở cấp project (403 `PERMISSION_DENIED`) dù nằm trong `client.models.list()` — dùng alias `-latest` để tài khoản tự resolve ra model nó thực sự được phép dùng, tránh phải đoán tên model lần thứ 3. "Thinking" bị tắt (`thinking_budget=0`) qua thử-rồi-lùi (gửi kèm tham số trước, bỏ đi nếu model từ chối) thay vì hardcode theo tên model — cho toàn bộ generate + 4 module judge, để:

> Ban đầu định dùng Gemini, nhưng tài khoản Google của người thực hiện đồ án bị yêu cầu khai báo billing mới cấp API key — đây là do lấy key qua Google Cloud Console/Vertex AI thay vì Google AI Studio. Đã tạm chuyển sang Groq (free tier xác nhận không cần billing) để không bị chặn tiến độ, sau đó **quay lại Gemini** (2026-08-26) dùng đúng luồng https://aistudio.google.com/apikey — luồng này xác nhận không yêu cầu billing.
...
2. `02_generator_baseline.ipynb` → dùng lại index từ bước 1, cần secret `GEMINI_API_KEY` (Colab Secrets, không hardcode trong notebook — lấy tại https://aistudio.google.com/apikey) → sinh `standard_rag_results.jsonl`, checkpoint theo từng câu nên an toàn khi bị ngắt session giữa chừng.
...
| Rate-limit API LLM (Gemini free tier, `retryDelay` vài giây trong lỗi 429) | Bình thường (per-minute) — `chat()` parse `retryDelay` từ lỗi để chờ đúng thời gian, tự qua |
| Rate-limit API LLM (Gemini free tier, `retryDelay` dài hoặc lặp lại liên tiếp nhiều câu hỏi) | Dấu hiệu quota ngày của **model đó** đã cạn, không phải per-minute. Notebook 02/03/04 tự phát hiện (`LONG_WAIT_THRESHOLD`) và chuyển sang model tiếp theo trong `CANDIDATE_MODELS` thay vì ngồi chờ. Nếu **toàn bộ** model trong danh sách đều bị khóa cùng lúc, không còn cách chờ trong phiên hiện tại — hạ `MAX_QUESTIONS`, chạy tiếp vào phiên/ngày khác |
| Gemini bắt setup billing mới cấp API key | Đã gặp thật — nguyên nhân là lấy key qua Google Cloud Console/Vertex AI thay vì Google AI Studio. Lấy key đúng luồng tại https://aistudio.google.com/apikey để tránh yêu cầu billing (đã xác nhận free tier không cần billing qua luồng này) |
| `404 model not found` cho toàn bộ `CANDIDATE_MODELS` khi vừa chuyển sang Gemini | Đã gặp thật ngay lần đầu (2026-08-26) — `gemini-2.0-flash`/`gemini-2.0-flash-lite` đã bị Google khai tử, xác nhận qua https://ai.google.dev/gemini-api/docs/models |
| `403 PERMISSION_DENIED: Your project has been denied access` cho model cụ thể (vd. `gemini-2.5-flash`, `gemini-3.5-flash`) **dù model đó có trong `client.models.list()` thật của tài khoản** | Đã gặp thật ngay sau khi sửa lỗi 404 ở trên (cùng ngày 2026-08-26) — đây là hạn chế cấp project/tài khoản, không phải sai tên model, nên đổi tên model cụ thể khác không giải quyết được gốc rễ. Sửa triệt để: ưu tiên alias `-latest` do Google tự quản lý (`gemini-flash-lite-latest`, `gemini-flash-latest`, `gemini-pro-latest`) trong `CANDIDATE_MODELS` — các alias này tự resolve ra model mà tài khoản/project đó thực sự được phép dùng, tránh phải đoán tên cụ thể lần nữa. Nếu alias `-latest` cũng bị chặn: đây là dấu hiệu tài khoản/project bị chặn toàn bộ (không phải riêng model) — cần tạo API key mới trong project mới hoàn toàn qua https://aistudio.google.com/apikey ("Create API key in new project"), hoặc liên hệ Google support như thông báo lỗi ghi rõ |
| Model từ chối tham số `thinking_config` (lỗi 400) | Không phải model nào cũng hỗ trợ tắt "thinking" qua tham số này. `chat()` tự thử gửi kèm trước, nếu bị từ chối thì gọi lại không kèm — không cần sửa code khi đổi model |
| **(Lịch sử, không còn áp dụng)** `403 ... is blocked at the project level` khi gọi `groq/compound`/`groq/compound-mini`; Groq báo `model_not_found`; `<think>...</think>` lẫn vào `generated_answer` từ model "thinking" `qwen/qwen3.6-27b` làm judge parse JSON sai âm thầm | Các sự cố này chỉ xảy ra trong giai đoạn dùng Groq (đã chuyển hẳn sang Gemini ngày 2026-08-26). Chi tiết đầy đủ giữ lại trong `CLAUDE.md` và `RESULTS_ANALYSIS.md` §2 vì vẫn cần để diễn giải các artifact cũ sinh ra từ giai đoạn Groq. `strip_think()` vẫn được giữ trong code Gemini như lớp phòng vệ vô hại, không phải vì Gemini có vấn đề tương tự (đã tắt hẳn "thinking" qua `thinking_budget=0`) |
| Một lỗi 403/4xx bất kỳ làm cả câu hỏi trả về rỗng dù `CANDIDATE_MODELS` còn model khác chưa thử | Bug thật đã sửa (từ giai đoạn Groq, vẫn áp dụng cho Gemini): nhánh lỗi không-retry-được phải `exhausted_models.add(model)` rồi `break` để nhảy sang model kế tiếp trong hàng đợi, không được `return ""` thoát thẳng khỏi hàm |
| Trộn lẫn dữ liệu Groq cũ và Gemini mới trong `final/artifacts/` | Nếu chuyển hẳn sang Gemini, xoá `standard_rag_results.jsonl`, `self_rag_results.jsonl`, `no_rag_results.jsonl`, `evaluation_details.jsonl`, `final_comparison_table.csv` trên Drive trước khi chạy lại notebook 02→03→04 — cơ chế resume-safe sẽ giữ nguyên các câu hỏi cũ (sinh bởi Groq) nếu không xoá, tạo ra bảng so sánh có hàng từ 2 nhà cung cấp khác nhau |
...
2. **Bảng so sánh định lượng 3 hệ thống** (`final_comparison_table.csv`, notebook 04) — trên phần giao nhau các câu đã chạy xong cả 3 hệ (không nhất thiết đủ 250 câu, tùy quota Gemini free tier còn lại khi chạy):
...
- [x] `02_generator_baseline.ipynb` — baseline RAG generator qua Gemini API (`google-genai`, model tự dò qua `client.models.list()`, ưu tiên `gemini-2.5-flash`, tự xoay vòng khi bị khóa quota dài hạn; đổi từ Gemini→Groq→**Gemini** — lịch sử đầy đủ ở `CLAUDE.md`), checkpoint JSONL resume-safe → `standard_rag_results.jsonl`.
- [x] `03_self_rag_pipeline.ipynb` — 4 module reflection (Retrieve-decision, ISREL batch, ISSUP, ISUSE) qua Gemini, sinh `self_rag_results.jsonl`.
- [x] `04_evaluation_report.ipynb` — sinh Hệ No-RAG, chấm Correctness/Support/Usefulness thống nhất cho cả 3 hệ trên phần giao nhau đã chạy xong, xuất `final_comparison_table.csv`. **Đang trong quá trình chuyển hẳn sang Gemini (2026-08-26) — cần xoá toàn bộ `standard_rag_results.jsonl`/`self_rag_results.jsonl`/`no_rag_results.jsonl`/`evaluation_details.jsonl`/`final_comparison_table.csv` cũ (sinh bởi Groq) trên Drive và chạy lại notebook 02→03→04 từ đầu trước khi dùng bất kỳ số liệu nào cho báo cáo, để tránh trộn lẫn dữ liệu 2 nhà cung cấp khác nhau.**
- [x] `RESULTS_ANALYSIS.md` — phân tích đầy đủ kết quả đã pull về (26/08/2026): số liệu retrieval + ablation `[ISREL]` dùng được ngay, phát hiện/sửa lỗi `<think>` khiến Correctness toàn bộ vô nghĩa, case study cụ thể, mapping vào khung báo cáo §28.
